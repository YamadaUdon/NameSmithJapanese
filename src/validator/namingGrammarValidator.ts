...
export class NamingGrammarValidator {
	...
	validateFunctionName(name: string): NamingIssue | null {
		...
		const isStartsWithVerb = this.isVerb(firstWord);

		// Check if function starts with verb
		if (!isStartsWithVerb) {
			// Might be wrong order (noun + verb)
			if (words.length > 1 && this.isVerb(words[1])) {
				...
				return {
					...
					severity: 'error',
					suggestion: this.toCamelCase(corrected),
					explanation: `関数は「動詞 → 対象」の順にしてください。例: ${this.toCamelCase(corrected)} (${name}ではなく)`
				};
			}

			// No verb found
			const suggested = this.suggestVerbForFunction(name);
			return {
				name: name,
				rule: 'Rule 5: 動作を表す場合',
				severity: 'error',
				suggestion: suggested,
				explanation: `関数は動詞から始まるべきです。例: ${suggested}`
			};
		}
